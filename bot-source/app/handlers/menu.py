...
async def show_resources_menu(
    callback: types.CallbackQuery,
    db_user: User,
    db: AsyncSession,
):
    """TZ Templar VPN: подменю «Наши каналы» — Новости / Гайды / Сайт."""
    if db_user is None:
        texts = get_texts(settings.DEFAULT_LANGUAGE)
        await callback.answer(
            texts.t(
    ...
    header = texts.t('RESOURCES_TITLE', '📢 <b>Наши каналы</b>')
    prompt = texts.t('RESOURCES_PROMPT', 'Выберите ресурс:')
    caption = f'{header}\n\n{prompt}' if prompt else header

    from app.utils.banners import show_with_banner  # noqa: PLC0415
    from app.keyboards.inline import get_resources_keyboard  # noqa: PLC0415
    await show_with_banner(
        callback=callback,
        banner_name='resources',
        caption=caption,
        keyboard=get_resources_keyboard(language=db_user.language),
        parse_mode='HTML',
        language=db_user.language,
    )
    await callback.answer()
...
async def process_language_change(
    ...
    if db_user.language.lower() == normalized_selected:
        await show_subscription_info(callback, db_user, db, skip_callback_answer=True)
    ...
    await show_subscription_info(callback, updated_user, db, skip_callback_answer=True)
    await callback.answer(texts.t('LANGUAGE_SELECTED', '🌐 Язык интерфейса обновлен.'))
