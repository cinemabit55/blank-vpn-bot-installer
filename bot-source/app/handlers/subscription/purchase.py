...
async def show_subscription_info(
    ...
    *,
    ...
    skip_callback_answer = skip_callback_answer or callback_auto_answered
    ...
    # Личный кабинет должен открываться быстро. Не ходим в RemnaWave на каждый
    # просмотр: живые проверки трафика/устройств вынесены в отдельные действия
    # и фоновые синхронизации. Здесь восстанавливаем подписку только если в БД
    # явно не хватает ссылки или UUID.
    sync_uuid = subscription.remnawave_uuid if settings.is_multi_tariff_enabled() else db_user.remnawave_uuid
    if not subscription.subscription_url or not sync_uuid:
    ...
    if show_devices:
        # Количество подключенных HWID тянется из RemnaWave и может заметно
        # задерживать весь экран. В обзоре показываем быстрый плейсхолдер,
        # а точные устройства остаются в отдельном разделе управления устройствами.
        devices_used_str = '—'
    ...
    try:
        ...
        if settings.is_tariffs_mode():
            try:
                ...
                if trial_tariff:
                    from app.database.crud.server_squad import get_effective_trial_squad_uuids

                    trial_traffic_limit = trial_tariff.traffic_limit_gb
                    trial_device_limit = trial_tariff.device_limit
                    trial_squads = await get_effective_trial_squad_uuids(db, trial_tariff.allowed_squads)
        ...
        if not trial_squads:
            ...
            eligible_squads = await get_trial_eligible_server_squads(db)
            trial_squads = [s.squad_uuid for s in eligible_squads]
        ...
        if remnawave_user and subscription_link:
            if settings.is_happ_cryptolink_mode():
                trial_success_text = (
                    ...
                    + texts.t(
                        ...
                        '📲 Нажмите на кнопку ниже, чтобы скачать приложение',
                    )
                )
                ...
                trial_success_text = (
                    ...
                    f'{texts.t("SUBSCRIPTION_IMPORT_INSTRUCTION_PROMPT", "📲 Нажмите на кнопку ниже, чтобы скачать приложение")}'
                )
            ...
            # TZ Templar VPN: блок выбора подключения заменён на одну кнопку "Скачать приложение"
...
async def handle_trial_payment_method(callback: types.CallbackQuery, db_user: User, db: AsyncSession):
    ...
    try:
        ...
        if settings.is_tariffs_mode():
            try:
                ...
                if trial_tariff:
                    ...
                    trial_squads_list = await get_effective_trial_squad_uuids(db, trial_tariff.allowed_squads)
                    tariff_id_for_trial = trial_tariff.id
                    tariff_trial_days = getattr(trial_tariff, 'trial_duration_days', None)
                    if tariff_trial_days:
                        trial_duration = tariff_trial_days
        ...
        if not trial_squads_list:
            ...
            eligible_squads = await get_trial_eligible_server_squads(db)
            trial_squads_list = [s.squad_uuid for s in eligible_squads]
...
def register_handlers(dp: Dispatcher):
    ...
    # TZ Templar VPN: меню "Скачать приложение" — выбор приложения + платформа (раздел 2.14)
    from app.handlers.subscription.apps import (
        handle_menu_apps,
        handle_apps_select,
        handle_apps_platform,
    )
    dp.callback_query.register(handle_menu_apps, F.data == 'menu_apps')
    dp.callback_query.register(handle_apps_select, F.data.startswith('apps_select_'))
    dp.callback_query.register(handle_apps_platform, F.data.startswith('apps_dl_'))
