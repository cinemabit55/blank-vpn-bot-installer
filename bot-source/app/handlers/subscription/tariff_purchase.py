...
from app.utils.timezone import ceil_days_until
...
def format_tariffs_list_text(
    ...
    from app.localization.texts import get_texts as _gt_th
    _texts_th = _gt_th(db_user.language if db_user else None)
    lines = [_texts_th.t('TARIFFS_HEADER', '📦 <b>Выберите тариф</b>')]
        ...
        if is_daily:
            ...
            if prices:
                ...
                if discount_percent > 0:
                    ...
                    discount_icon = '🔥'
                price_text = f'{format_price_kopeks(min_price, compact=True)}{discount_icon}'

        # Компактный формат: Название — 250 ГБ / 10 📱 от 179₽🔥
        purchased_mark = ' ✅' if tariff.id in purchased_tariff_ids else ''
        lines.append(
            f'<b>{html.escape(tariff.name)}</b>{purchased_mark} — {price_text}'
        )

        # TZ Templar VPN: описание скрыто на экране выбора тарифа (остаётся на экране деталей)
...
async def show_tariff_switch_list(
    ...
    if current_tariff:
        remaining_days = ceil_days_until(subscription.end_date)
...
async def select_tariff_switch(
    ...
    if current_subscription_sw and current_subscription_sw.tariff_id:
        ...
        if cur_tariff_sw:
            rem_days = ceil_days_until(current_subscription_sw.end_date)
...
async def confirm_tariff_switch(
    ...
    if subscription.tariff_id and subscription.tariff_id != tariff_id:
        ...
        if cur_tariff_obj:
            rem_days = ceil_days_until(subscription.end_date)
