...
# aiogram logs transient polling/network problems at ERROR level. They are
# self-healing and should stay in logs without paging admins.
TRANSIENT_LOGGER_PREFIXES: Final[tuple[str, ...]] = (
    'aiogram.dispatcher',
)
TRANSIENT_MESSAGE_PATTERNS: Final[tuple[str, ...]] = (
    'failed to fetch updates',
    'telegramnetworkerror',
    'request timeout error',
    'connection timeout',
)
TRANSIENT_EXCEPTION_TYPES: Final[set[str]] = {
    'TelegramNetworkError',
...
class TelegramNotifierProcessor:
    ...
    def __call__(
        ...
        # 3. Filter noisy transport-level loggers
        ...
        #   (a) stdlib LogRecord.exc_info for logging-originated records
        #   (b) sys.exc_info() — works when logger.error is called inside except
        #   (c) error/exc/exception kwargs if they carry __traceback__
        ...
        if exc_info is True:
            ...
            record_exc_info = getattr(record, 'exc_info', None) if record is not None else None
            if record_exc_info:
                event_dict['exc_info'] = record_exc_info
            else:
        ...
        if _is_transient_network_log(logger_name, _extract_event_message(event_dict), event_dict):
            return event_dict
    ...
    def _compute_hash(event_dict: dict[str, Any]) -> str:
        ...
        """
        logger_name = event_dict.get('logger', '')
        event_msg = _extract_event_message(event_dict)
...
def _extract_event_message(event_dict: dict[str, Any]) -> str:
    """Return a human-readable message for structlog and stdlib records."""
...
def _get_exception_type_name(event_dict: dict[str, Any]) -> str:
    exc_info = event_dict.get('exc_info')
    if exc_info and isinstance(exc_info, tuple) and exc_info[0] is not None:
...
def _is_transient_network_log(logger_name: str, message: str, event_dict: dict[str, Any]) -> bool:
    if not any(logger_name.startswith(prefix) for prefix in TRANSIENT_LOGGER_PREFIXES):
        return False

    lowered = message.lower()
    if any(pattern in lowered for pattern in TRANSIENT_MESSAGE_PATTERNS):
        return True

    return _get_exception_type_name(event_dict) in TRANSIENT_EXCEPTION_TYPES
...
def _make_event_dict_error(event_dict: dict[str, Any]) -> Exception:
    ...
    message = _extract_event_message(event_dict) or '(empty error log message)'
