...
async def create_trial_subscription(
    ...
    # Иначе используем squad_uuid или trial pool по умолчанию.
    final_squads = []
    if connected_squads:
        ...
        try:
            ...
            final_squads = await get_effective_trial_squad_uuids(db)
            if final_squads:
                logger.debug(
...
async def add_bonus_days_to_user(
    db,
    user,
    days: int,
    *,
    ...
    """Добавляет N дней к активной подписке инвайтера; если её нет — копит в users.pending_bonus_days.

    Возвращает кортеж (granted_now: bool, total_pending: int).
    granted_now=True означает что дни были применены к существующей подписке.
    granted_now=False означает что дни ушли в pending (нет активной подписки).
    """
    from app.database.models import SubscriptionStatus
    if days <= 0:
        return (False, int(getattr(user, 'pending_bonus_days', 0) or 0))
    ...
    if active is not None:
        await extend_subscription(db, active, days=days, commit=commit)
        return (True, int(getattr(user, 'pending_bonus_days', 0) or 0))

    new_total = int(getattr(user, 'pending_bonus_days', 0) or 0) + int(days)
    user.pending_bonus_days = new_total
    # Гарантированный flush в БД через явный UPDATE — не зависит от того,
    # что произойдёт с инстансом user далее в сессии (рефреш и т.п.)
    from app.database.models import User as _UserModel
    await db.execute(
        update(_UserModel).where(_UserModel.id == user.id).values(pending_bonus_days=new_total)
    )
    if commit:
        await db.commit()
