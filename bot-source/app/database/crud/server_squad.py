...
async def get_effective_trial_squad_uuids(
    db: AsyncSession,
    allowed_squads: Sequence[str] | None = None,
) -> list[str]:
    """Resolve trial squads, preferring the explicit trial pool when unrestricted."""

    normalized = [str(squad_uuid) for squad_uuid in (allowed_squads or []) if squad_uuid]
    if normalized:
        return list(dict.fromkeys(normalized))

    trial_squads = await get_trial_eligible_server_squads(db)
    if trial_squads:
        return [squad.squad_uuid for squad in trial_squads if squad.squad_uuid]
