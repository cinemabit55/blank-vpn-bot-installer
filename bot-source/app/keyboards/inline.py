...
# --- TEMPLAR PAYMENT BUTTONS FINAL START ---
_TEMPLAR_ORIGINAL_INLINE_KEYBOARD_MARKUP = InlineKeyboardMarkup

def _templar_payment_inline_markup(*args, **kwargs):
    """
    Перехватывает создание InlineKeyboardMarkup.
    Меняет только клавиатуру выбора способа оплаты:
    1. 🏦 СБП
    2. 💳 Банковская карта
    3. 🪙 Криптовалюта #1
    4. 🪙 Криптовалюта #2
    Удаляет кнопку поддержки.
    """
    try:
        rows = None
        ...
        if not isinstance(rows, list):
            return _TEMPLAR_ORIGINAL_INLINE_KEYBOARD_MARKUP(*args, **kwargs)

        labels = {
            "platega_m2": "🏦 СБП",
            "tribute": "💳 Банковская карта",
            "platega_m13": "🪙 Криптовалюта #1",
            "cryptobot": "🪙 Криптовалюта #2",
        }

        order = ["platega_m2", "tribute", "platega_m13", "cryptobot"]
        ...
        def button_text(button):
            return (getattr(button, "text", "") or "").lower()

        def button_callback(button):
            return (getattr(button, "callback_data", "") or "").lower()

        def detect_method(button):
            cb = button_callback(button)
            label = button_text(button)

            if "platega_m2" in cb or "сбп" in label or "sbp" in cb:
                return "platega_m2"

            if "platega_m13" in cb:
                return "platega_m13"

            if "tribute" in cb or "tribute" in label or "банковская карта" in label:
                return "tribute"

            if "cryptobot" in cb or "crypto_bot" in cb or "cryptobot" in label:
                return "cryptobot"

            # Осторожный fallback: запасная крипта без Tribute почти всегда CryptoBot.
            if "криптовалюта" in label and "запас" in label:
                return "cryptobot"

            # Если это крипта через Platega, но код почему-то спрятан.
            if "криптовалюта" in label and "platega" in label:
                return "platega_m13"

            return None

        def is_support(button):
            cb = button_callback(button)
            label = button_text(button)
            ...
            return any(word in cb or word in label for word in bad_words)

        def replace_button_text(button, new_text):
            ...
            try:
                ...
                if hasattr(button, "copy"):
                    return button.copy(update={"text": new_text})
            except Exception:
                pass

            try:
                if cb:
                    return InlineKeyboardButton(text=new_text, callback_data=cb)
                if url:
                    return InlineKeyboardButton(text=new_text, url=url)
            except Exception:
                return button

            return button

        method_rows = {code: [] for code in order}
        other_rows = []
        found_payment_keyboard = False

        for row in rows:
            button = get_button(row)

            if button is None:
                other_rows.append(row)
                continue

            method = detect_method(button)

            if method:
                found_payment_keyboard = True
                method_rows[method].append([replace_button_text(button, labels[method])])
                continue

            if is_support(button):
                # Удаляем поддержку только если это реально клавиатура оплаты.
                # Если позже выяснится, что методов оплаты нет, восстановим оригинал.
                continue

            other_rows.append(row)

        if not found_payment_keyboard:
            return _TEMPLAR_ORIGINAL_INLINE_KEYBOARD_MARKUP(*args, **kwargs)

        new_rows = []
        for code in order:
            new_rows.extend(method_rows[code])

        # Оставляем "Назад" и любые служебные кнопки после методов.
        new_rows.extend(other_rows)

        if "inline_keyboard" in kwargs:
            kwargs["inline_keyboard"] = new_rows
            return _TEMPLAR_ORIGINAL_INLINE_KEYBOARD_MARKUP(*args, **kwargs)
        ...
        return _TEMPLAR_ORIGINAL_INLINE_KEYBOARD_MARKUP(inline_keyboard=new_rows)

    except Exception:
        return _TEMPLAR_ORIGINAL_INLINE_KEYBOARD_MARKUP(*args, **kwargs)
# --- TEMPLAR PAYMENT BUTTONS FINAL END ---

def get_rules_keyboard(language: str = DEFAULT_LANGUAGE) -> InlineKeyboardMarkup:
    texts = get_texts(language)
    return _templar_payment_inline_markup(
        inline_keyboard=[
            [
...
def _build_cabinet_main_menu_keyboard(
    ...
    *,
    ...
    for row_key in row_keys:
        ...
        for btn_id in btn_ids:
            ...
            match btn_id:
                ...
                case 'language':
                    # Language switching lives in the subscription cabinet, not in the main menu.
                    continue
    ...
    return _templar_payment_inline_markup(inline_keyboard=keyboard_rows)
...
def get_main_menu_keyboard(
    ...
    *,
    ...
    if has_active_subscription and subscription_is_active:
        # TZ Templar VPN: убраны кнопки "Подключиться" и "Подписка" при активной подписке
        # — "Подключиться" дублируется на экране Личного кабинета
        # — "Подписка" дублирует кнопку "Личный кабинет" из paired_buttons (тот же callback menu_subscription)
    ...
    # TZ Templar VPN: кнопка Баланса убрана — дублирует информацию из Личного кабинета и текста меню
    ...
    # Кнопки ТЗ Templar VPN: Личный кабинет + Скачать приложение
    # При активной подписке/триале: ЛК соло-рядом, ниже [Продлить] + [Скачать].
    # Без активной подписки — обе кнопки идут в общий paired-поток (как раньше),
    # а кнопка "Купить" автоматически добавляется блоком show_buy выше.
    if has_active_subscription and subscription_is_active:
        # Сбрасываем накопленный paired_buttons (например, "Докупить трафик")
        # в keyboard ПЕРЕД ЛК, чтобы он не сломал раскладку соло-ряда.
        if paired_buttons:
            for _i in range(0, len(paired_buttons), 2):
                keyboard.append(paired_buttons[_i : _i + 2])
            paired_buttons = []
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=texts.t('MENU_PROFILE', '👤 Личный кабинет'),
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=texts.t('MENU_RENEW_SUBSCRIPTION', '🔄 Продлить подписку'),
                    callback_data='subscription_upgrade',
                ),
                InlineKeyboardButton(
                    text=texts.t('MENU_DOWNLOAD_APP', '📲 Скачать приложение'),
    ...
    # TZ Templar VPN: вместо «О сервисе» в парном ряду — кнопка «Канал».
    # «О сервисе» добавляется отдельным рядом ниже (после построения пар).
    # TZ Templar VPN: «Наши каналы» — подменю с Новостями/Гайдами/Сайтом
    paired_buttons.append(
        InlineKeyboardButton(
            text=texts.t('MENU_RESOURCES', '📢 Наши каналы'),
    ...
    # TZ Templar VPN: «О сервисе» — отдельный ряд под парными кнопками
    keyboard.append(
        [
            InlineKeyboardButton(
                text=texts.t('MENU_INFO', 'ℹ️ О сервисе'),
    ...
    return _templar_payment_inline_markup(inline_keyboard=keyboard)
...
def get_back_keyboard(language: str = DEFAULT_LANGUAGE, callback_data: str = 'back_to_menu') -> InlineKeyboardMarkup:
    texts = get_texts(language)
    return _templar_payment_inline_markup(inline_keyboard=[[InlineKeyboardButton(text=texts.BACK, callback_data=callback_data)]])
...
def get_subscription_keyboard(
    ...
    if has_subscription:
        # TZ Templar VPN: блок «Подключиться» в ЛК заменён на одну кнопку «Скачать приложение».
        # Прямой запуск конфига убран — пользователь сначала идёт в раздел приложений.
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=texts.t('MENU_DOWNLOAD_APP', '📲 Скачать приложение'),
...
def get_referral_keyboard(language: str = DEFAULT_LANGUAGE) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        # TZ Templar VPN: кнопка «Создать приглашение» убрана из меню партнёрки (хендлер оставлен для совместимости)
        ...
        # TZ Templar VPN: кнопка «Аналитика» убрана из меню партнёрки (хендлер оставлен для совместимости)
    ]
...
def get_support_keyboard(language: str = DEFAULT_LANGUAGE) -> InlineKeyboardMarkup:
    """Support screen with a contact button."""
    texts = get_texts(language)
    contact_url = settings.get_support_contact_url() or 'https://t.me/example'
    rows = [
        [
            InlineKeyboardButton(
    ...
    return _templar_payment_inline_markup(inline_keyboard=rows)
