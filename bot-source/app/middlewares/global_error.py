...
async def send_error_to_admin_chat(
    ...
    try:
        ...
        # Telegram caption limit is 1024; reserve headroom for context/recommendations/timestamp.
        caption_message_preview = error_message or '(no message)'
        if len(caption_message_preview) > 350:
            caption_message_preview = caption_message_preview[:350] + '…'
        ...
        if context:
            message_text += f'<b>Контекст:</b> {_html_escape(context)}\n'
