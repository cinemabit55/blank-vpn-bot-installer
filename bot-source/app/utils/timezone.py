...
def ceil_days_until(end_dt: datetime | None, now: datetime | None = None) -> int:
    """Return billable days until ``end_dt``, rounding any positive remainder up.

    Tariff switches use whole-day pricing. ``timedelta.days`` floors partial
    days to zero, which can make a same-day upgrade free even while the
    ...
    aware_end = end_dt if end_dt.tzinfo is not None else end_dt.replace(tzinfo=UTC)
    aware_now = current if current.tzinfo is not None else current.replace(tzinfo=UTC)
    seconds_left = (aware_end - aware_now).total_seconds()
    if seconds_left <= 0:
        return 0
    return max(1, ceil(seconds_left / 86400))
