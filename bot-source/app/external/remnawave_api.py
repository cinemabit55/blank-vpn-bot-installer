...
RETRYABLE_HTTP_STATUS_CODES = frozenset({429, 500, 502, 503, 504, 520, 521, 522, 523, 524})
TRANSIENT_HTTP_STATUS_CODES = RETRYABLE_HTTP_STATUS_CODES
...
def is_transient_remnawave_error(error: BaseException) -> bool:
...
class RemnaWaveAPI:
            ...
            try:
                ...
                async with self.session.request(method, **kwargs) as response:
                    ...
                    if response.status >= 400:
                        ...
                        log = (
                            logger.warning
                            if response.status in TRANSIENT_HTTP_STATUS_CODES or is_harmless
                            else logger.error
                        )
                ...
                if attempt < max_retries:
                    ...
                    continue
                logger.warning('Request failed after retries', error=e)
                raise RemnaWaveAPIError(f'Request failed: {e!s}') from e

        raise RemnaWaveAPIError(f'Max retries exceeded for {method} {endpoint}')


    async def _retry_user_mutation_without_invalid_relations(
        self,
        method: str,
        endpoint: str,
        data: dict,
        ...
        **log_context,
        ...
        """Retry user create/update after dropping stale squad relation fields."""
        error_code = (error.response_data or {}).get('errorCode', '')
        fields_to_drop: list[str] = []

        if error_code == 'A039' and 'externalSquadUuid' in data:
            fields_to_drop.append('externalSquadUuid')
        if error_code in {'A018', 'A039'} and 'activeInternalSquads' in data:
            fields_to_drop.append('activeInternalSquads')
        ...
        for field in fields_to_drop:
            ...
            dropped_summary[field] = len(value) if isinstance(value, list) else bool(value)

        logger.warning(
            'RemnaWave rejected user relation fields, retrying without them',
            method=method,
            endpoint=endpoint,
            error_code=error_code,
            dropped_fields=fields_to_drop,
    ...
    async def create_user(
        ...
        try:
            ...
            try:
                response = await self._retry_user_mutation_without_invalid_relations(
                    'POST',
                    '/api/users',
                    data,
                    e,
