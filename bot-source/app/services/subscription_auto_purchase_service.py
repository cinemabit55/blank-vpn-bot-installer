...
async def _auto_extend_subscription(
    ...
    *,
    ...
    # TZ Templar VPN — Кусок 2 (auto-extend): +10% дней инвайтеру с автопродления подписки реферала
    try:
        from app.services.referral_service import process_referral_purchase_days
        await process_referral_purchase_days(
            db,
            user.id,
            prepared.period_days,
            bot=bot,
        ...
        logger.error('Хук реферальной премии в днях (auto-extend) упал', error=_ref_err)
...
async def _auto_purchase_tariff(
    ...
    *,
    ...
    # TZ Templar VPN — Кусок 2 (auto-purchase tariff): +10% дней инвайтеру с автопокупки тарифа реферала
    try:
        from app.services.referral_service import process_referral_purchase_days
        await process_referral_purchase_days(
            db,
        ...
        logger.error('Хук реферальной премии в днях (auto-purchase tariff) упал', error=_ref_err)
...
async def try_auto_extend_expired_after_topup(
    ...
    *,
    ...
    # TZ Templar VPN — Кусок 2 (auto-extend expired): +10% дней инвайтеру с автопродления просроченной подписки реферала
    try:
        from app.services.referral_service import process_referral_purchase_days
        await process_referral_purchase_days(
            db,
        ...
        logger.error('Хук реферальной премии в днях (auto-extend expired) упал', error=_ref_err)
