    ...
    async def _send_backup_file_to_chat(self, file_path: str):
        ...
        try:
            ...
            if not chat_id:
                return
            if self.bot is None:
                logger.warning('Отправка бекапа в чат пропущена: bot не инициализирован')
                return
            ...
            if _is_backup_chat_unreachable(e):
                logger.warning(
                    'Отправка бекапа в чат пропущена: Telegram destination недоступен',
                    chat_id=settings.get_backup_send_chat_id(),
                    topic_id=settings.BACKUP_SEND_TOPIC_ID,
                    error=e,
                )
            else:
                logger.error('Ошибка отправки бекапа в чат', error=e, exc_info=True)
        except Exception as e:
            logger.error('Ошибка отправки бекапа в чат', error=e, exc_info=True)
        finally:
...
def _is_backup_chat_unreachable(error: Exception) -> bool:
    ...
    return isinstance(error, TelegramForbiddenError) or any(marker in message for marker in markers)
