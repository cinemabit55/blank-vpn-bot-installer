    ...
    async def submit_purchase(
        ...
        try:
            ...
            await process_referral_purchase_days(
                db,
                user.id,
                int(pricing.selection.period.days),
                bot=getattr(self, 'bot', None),
            )
        except Exception as _ref_err:
            logger.error('Хук реферальной премии в днях упал', error=_ref_err)

        try:
            if getattr(user, 'promo_offer_discount_source', None) == 'referral_first_purchase' \
               and int(getattr(user, 'promo_offer_discount_percent', 0) or 0) > 0:
                user.promo_offer_discount_percent = 0
                user.promo_offer_discount_source = None
                user.promo_offer_discount_expires_at = None
                await db.commit()
                logger.info('Скидка приглашённому израсходована', user_id=user.id)
        except Exception as _disc_err:
            logger.error('Не удалось сбросить скидку приглашённого', error=_disc_err)
