...
"""
    ...
    async def _sync_with_remnawave(self, db: AsyncSession):
        try:
            ...
            if is_transient_remnawave_error(e):
                logger.warning('Временная недоступность RemnaWave во время мониторинга', error=e, status_code=e.status_code)
            else:
