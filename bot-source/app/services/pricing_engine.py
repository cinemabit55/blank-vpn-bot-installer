...
class PricingEngine:
    ...
    def calculate_tariff_switch_cost(
        ...
        *,
        ...
        current_tier = int(getattr(current_tariff, 'tier_level', 0) or 0)
        new_tier = int(getattr(new_tariff, 'tier_level', 0) or 0)
        is_tier_upgrade = new_tier > current_tier

        if numerator > 0:
            raw_cost = max(1, numerator // denominator)
        elif is_tier_upgrade and new_price > 0:
            # A higher business tier is an upgrade even when prices are equal
            # or misconfigured. Charge a small one-day unlock fee instead of
            # silently treating it as a free downgrade/same-tier switch.
            raw_cost = max(1, (new_price + new_period - 1) // new_period)
        else:
            return TariffSwitchResult(
                upgrade_cost=0,
                is_upgrade=False,
