...
class SubscriptionService:
    ...
    async def _filter_existing_internal_squads(
        self,
        api: RemnaWaveAPI,
        ...
        *,
        ...
        """Drop stale internal squad UUIDs before sending a user payload to RemnaWave."""
        requested = [str(uuid) for uuid in (squad_uuids or []) if uuid]
        if not requested:
            return []

        try:
            live_squads = await api.get_internal_squads()
        except Exception as error:
            logger.warning(
                'Не удалось проверить internal squads в RemnaWave, отправляем исходный список',
        ...
        live_uuids = {squad.uuid for squad in live_squads}
        filtered = [uuid for uuid in requested if uuid in live_uuids]
        if len(filtered) != len(requested):
            logger.warning(
                'Убраны устаревшие RemnaWave internal squads из payload',
                subscription_id=subscription_id,
                user_id=user_id,
                dropped_count=len(requested) - len(filtered),
    ...
    async def _create_or_update_remnawave_user_multi(
        ...
        *,
        ...
        active_squads = await self._filter_existing_internal_squads(
            api,
            subscription.connected_squads,
            subscription_id=subscription.id,
            user_id=user.id,
        )
        if active_squads != list(subscription.connected_squads or []):
            subscription.connected_squads = active_squads
        ...
        if active_squads:
            common_kwargs['active_internal_squads'] = active_squads
        ...
        *,
        ...
        try:
            ...
            async with self.get_api_client() as api:
                ...
                if sync_squads:
                    requested_squads = list(subscription.connected_squads or [])
                    active_squads = await self._filter_existing_internal_squads(
                        api,
                        requested_squads,
                        subscription_id=subscription.id,
                    ...
                    if active_squads != requested_squads:
                        subscription.connected_squads = active_squads
                    if requested_squads or active_squads:
                        update_kwargs['active_internal_squads'] = active_squads
