...
async def process_referral_registration(db: AsyncSession, new_user_id: int, referrer_id: int, bot: Bot = None):
    try:
        ...
        # Кусок 3: скидка приглашённому на первую покупку (без срока, до первой покупки)
        try:
            _invited_pct = int(getattr(settings, 'REFERRAL_INVITED_DISCOUNT_PERCENT', 0) or 0)
            if _invited_pct > 0 and new_user is not None:
                new_user.promo_offer_discount_percent = _invited_pct
                new_user.promo_offer_discount_source = 'referral_first_purchase'
                new_user.promo_offer_discount_expires_at = None
                await db.commit()
                logger.info(
                    'Скидка приглашённому записана',
                    user_id=new_user_id,
                    discount_percent=_invited_pct,
                )
        except Exception as _disc_exc:
            logger.error('Не удалось записать скидку приглашённому', error=_disc_exc)

        if bot:
            commission_percent = get_effective_referral_commission_percent(referrer)
            new_user_texts = get_texts(getattr(new_user, 'language', None) or 'ru')
            referrer_texts = get_texts(getattr(referrer, 'language', None) or 'ru')
            referral_notification = new_user_texts.t(
                'REFERRAL_WELCOME_HEADER',
                '🎉 <b>Добро пожаловать!</b>\n\nВы перешли по реферальной ссылке пользователя <b>{referrer_name}</b>!',
            ).format(referrer_name=html.escape(referrer.full_name))
            _w_invited_pct = int(getattr(settings, 'REFERRAL_INVITED_DISCOUNT_PERCENT', 0) or 0)
            if _w_invited_pct > 0:
                referral_notification += new_user_texts.t(
                    'REFERRAL_WELCOME_DISCOUNT',
                    '\n\n🎁 Вам начислена <b>скидка {percent}%</b> на первую покупку подписки!',
                ).format(percent=_w_invited_pct)
            elif settings.REFERRAL_FIRST_TOPUP_BONUS_KOPEKS > 0:
                referral_notification += new_user_texts.t(
                    'REFERRAL_WELCOME_FIRST_TOPUP_BONUS',
                    '\n\n💰 При первом пополнении от {min_topup} вы получите бонус {bonus}!',
                ).format(
            ...
            inviter_notification = referrer_texts.t(
                'REFERRAL_INVITER_HEADER',
                '👥 <b>Новый реферал!</b>\n\nПо вашей ссылке зарегистрировался пользователь <b>{referral_name}</b>!\n\n💰 Когда он пополнит баланс от {min_topup}, ',
            ).format(
                referral_name=html.escape(new_user.full_name),
                min_topup=settings.format_price(settings.REFERRAL_MINIMUM_TOPUP_KOPEKS),
            )
            inviter_days = int(getattr(settings, 'REFERRAL_INVITER_DAYS_BONUS', 0) or 0)
            if inviter_days > 0:
                inviter_notification += referrer_texts.t(
                    'REFERRAL_INVITER_REWARD_DAYS',
                    'вы получите <b>+{days} дн.</b> к вашей подписке (если активной подписки нет — дни накопятся и добавятся при следующей покупке).\n\n',
                ).format(days=inviter_days)
            elif settings.REFERRAL_INVITER_BONUS_KOPEKS > 0 and commission_percent > 0:
                inviter_notification += referrer_texts.t(
                    'REFERRAL_INVITER_REWARD_BONUS_AND_COMMISSION',
                    'вы получите {bonus} + {commission_percent}% от суммы пополнения.\n\n',
                ).format(
                    bonus=settings.format_price(settings.REFERRAL_INVITER_BONUS_KOPEKS),
                    commission_percent=commission_percent,
                )
            elif settings.REFERRAL_INVITER_BONUS_KOPEKS > 0:
                inviter_notification += referrer_texts.t(
                    'REFERRAL_INVITER_REWARD_BONUS_ONLY',
                    'вы получите {bonus}.\n\n',
                ).format(bonus=settings.format_price(settings.REFERRAL_INVITER_BONUS_KOPEKS))
            elif commission_percent > 0:
                inviter_notification += referrer_texts.t(
                    'REFERRAL_INVITER_REWARD_COMMISSION_ONLY',
                    'вы получите {commission_percent}% от суммы.\n\n',
                ).format(commission_percent=commission_percent)
            else:
                inviter_notification += referrer_texts.t(
                    'REFERRAL_INVITER_REWARD_NOTIFY_ONLY',
                    'вы получите уведомление.\n\n',
                )
            purchase_pct = int(getattr(settings, 'REFERRAL_INVITER_PURCHASE_PERCENT', 0) or 0)
            if purchase_pct > 0:
                inviter_notification += referrer_texts.t(
                    'REFERRAL_INVITER_FOOTER_PURCHASE_DAYS',
                    '📈 С каждой его покупки подписки — <b>+{percent}%</b> дней от срока к вашей подписке.',
                ).format(percent=purchase_pct)
            elif commission_percent > 0:
                inviter_notification += referrer_texts.t(
                    'REFERRAL_INVITER_FOOTER_COMMISSION',
                    '📈 С каждой последующей оплаты — <b>{percent}%</b> комиссии на баланс.',
        ...
        if not user.has_made_first_topup:
            ...
            if settings.REFERRAL_FIRST_TOPUP_BONUS_KOPEKS > 0:
                ...
                if bonus_ok:
                    ...
                    if bot:
                        _bonus_texts = get_texts(getattr(user, 'language', None) or 'ru')
                        bonus_notification = _bonus_texts.t(
                            'REFERRAL_BONUS_RECEIVED',
                            '🎉 <b>Бонус получен!</b>\n\nЗа первое пополнение вы получили бонус {bonus}!\n\n💎 Средства зачислены на ваш баланс.',
                        ).format(bonus=settings.format_price(settings.REFERRAL_FIRST_TOPUP_BONUS_KOPEKS))
                        await send_referral_notification(
                            bot,
                            user.telegram_id,
                            bonus_notification,
            ...
            commission_amount = int(topup_amount_kopeks * commission_percent / 100)
            # ── Кусок 1: первый бонус инвайтеру в днях ─────────────────────────
            if settings.REFERRAL_INVITER_DAYS_BONUS > 0:
                from app.database.crud.subscription import add_bonus_days_to_user
                _granted_now, _pending_total = await add_bonus_days_to_user(
                    db,
                    referrer,
                    settings.REFERRAL_INVITER_DAYS_BONUS,
                    commit=True,
                )
                logger.info(
                    'DEBUG: после add_bonus_days_to_user',
                    granted_now=_granted_now, pending_total=_pending_total, referrer_id=referrer.id,
                )
                if bot:
                    if _granted_now:
                        _days_msg = (
                            f'🎉 <b>Бонус за приглашение!</b>\n\n'
                            f'Ваш реферал <b>{html.escape(user.full_name)}</b> '
                            f'совершил первую оплату.\n\n'
                            f'🎁 К вашей подписке добавлено '
                            f'<b>{settings.REFERRAL_INVITER_DAYS_BONUS} дн.</b>'
                        )
                    else:
                        _days_msg = (
                            f'🎉 <b>Бонус за приглашение!</b>\n\n'
                            f'Ваш реферал <b>{html.escape(user.full_name)}</b> '
                            f'совершил первую оплату.\n\n'
                            f'🎁 Вам начислено <b>{settings.REFERRAL_INVITER_DAYS_BONUS} дн.</b> '
                            f'бонусом — будут добавлены к подписке при следующей покупке '
                            f'(всего накоплено: <b>{_pending_total} дн.</b>)'
                        )
                    await send_referral_notification(
                        bot,
                        referrer.telegram_id,
            ...
            if inviter_bonus > 0:
                ...
                if balance_ok:
                    ...
                    if bot:
                        ...
                        _purchase_pct = int(getattr(settings, 'REFERRAL_INVITER_PURCHASE_PERCENT', 0) or 0)
                        if _purchase_pct > 0:
                            inviter_bonus_notification += (
                                f'\n\n📈 Теперь с каждой его покупки подписки '
                                f'вы будете получать +{_purchase_pct}% дней от срока к своей подписке.'
                            )
                        elif commission_percent > 0:
                            inviter_bonus_notification += (
...
async def process_referral_purchase_days(
    db, user_id: int, days_purchased: int, bot=None,
):
    """Кусок 2: начисляет инвайтеру X% дней от срока подписки реферала.
    Срабатывает на платной покупке/продлении. Дни идут в активную подписку инвайтера,
    либо копятся в pending_bonus_days до следующей его покупки.
    """
    try:
        from app.database.crud.user import get_user_by_id
        from app.database.crud.subscription import add_bonus_days_to_user

        days_purchased = int(days_purchased or 0)
        if days_purchased <= 0:
            return False

        user = await get_user_by_id(db, user_id)
        if user is None or not user.referred_by_id:
            return False

        percent = int(getattr(settings, 'REFERRAL_INVITER_PURCHASE_PERCENT', 0) or 0)
        if percent <= 0:
            return False

        inviter_days = (days_purchased * percent) // 100
        if inviter_days <= 0:
            return False
        ...
        granted_now, pending_total = await add_bonus_days_to_user(
            db, referrer, inviter_days, commit=True,
        )
        logger.info(
            'Реферальная премия в днях начислена',
            referrer_id=referrer.id,
        ...
        if bot:
            try:
                if granted_now:
                    msg = (
                        f'🎁 <b>Бонус с покупки реферала!</b>\n\n'
                        f'Ваш реферал <b>{html.escape(user.full_name)}</b> купил подписку на '
                        f'<b>{days_purchased} дн.</b>\n\n'
                        f'➕ К вашей подписке добавлено <b>{inviter_days} дн.</b> '
                        f'({percent}% от срока).'
                    )
                else:
                    msg = (
                        ...
                        f'Ваш реферал <b>{html.escape(user.full_name)}</b> купил подписку на '
                        f'<b>{days_purchased} дн.</b>\n\n'
                        f'📦 Накоплено <b>{inviter_days} дн.</b> ({percent}% от срока) — '
                        f'добавятся к вашей подписке при следующей покупке.'
                    )
                await send_referral_notification(
                    bot, referrer.telegram_id, msg,
                    user=referrer, referral_name=user.full_name,
                )
            except Exception as e:
                logger.error('Уведомление инвайтеру не отправлено', error=e)

        return True
    except Exception as e:
        logger.error('Ошибка process_referral_purchase_days', error=e)
        import traceback
