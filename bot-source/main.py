...
from app.services.subscription_expiry_notifications import subscription_expiry_notifications_worker
...
async def main():
    ...
    timeline = StartupTimeline(logger, 'VPN Bot')
    timeline.log_banner(
        [
    ...
    subscription_expiry_notifications_task = None
    polling_task = None
    web_api_server = None
    ...
    try:
        ...
        subscription_expiry_notifications_task = asyncio.create_task(
            subscription_expiry_notifications_worker(bot)
        )
        logger.info('Subscription expiry notifications task scheduled')
        ...
        async with timeline.stage(
            ...
            if polling_enabled:
                drop_pending_updates = settings.WEBHOOK_DROP_PENDING_UPDATES is not False
                await bot.delete_webhook(drop_pending_updates=drop_pending_updates)
                polling_task = asyncio.create_task(dp.start_polling(bot))
                stage.log(
                    'drop_pending_updates=True — старые обновления отброшены'
                    if drop_pending_updates
                    else 'drop_pending_updates=False — накопившиеся обновления будут обработаны'
                )
            else:
                polling_task = None
        ...
        if subscription_expiry_notifications_task:
            logger.info('Остановка задачи уведомлений об окончании подписок...')
            subscription_expiry_notifications_task.cancel()
            try:
                await subscription_expiry_notifications_task
            except asyncio.CancelledError:
                pass
        ...
        if telegram_webhook_enabled and 'bot' in locals():
            ...
            try:
                await bot.delete_webhook(drop_pending_updates=bool(settings.WEBHOOK_DROP_PENDING_UPDATES))
