...
Скрипты добавления нод берутся из актуального репозитория бота. В них должны быть заложены последние настройки: TCP REALITY по умолчанию, XHTTP только как совместимый старый режим, server keepalive, Firefox/Safari SNI по выбранному сценарию, HAPP-заголовки, фрагментация, общий no-limit для TCP, маршрутизация RU direct, Whoosh direct и UDP direct/WARP-логика из текущей версии бота.
...
После установки бот запускается без баннеров. Оператор может при необходимости добавить собственные изображения.

Новостной канал задается необязательным полем `news_channel_username` при установке. Его можно оставить пустым и позже изменить через настройки конфигурации бота (`NEWS_CHANNEL_USERNAME`).
